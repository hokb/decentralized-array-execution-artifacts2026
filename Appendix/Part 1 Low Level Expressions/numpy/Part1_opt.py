...
result_filename = "results_numpy_opt.txt" if len(sys.argv) <= 2 else sys.argv[2] 
times_filename = "times_numpy_opt.txt" if len(sys.argv) <= 1 else sys.argv[1] 
...
with open(times_filename, "w") as timesfile:
    with open(result_filename, "wb") as resultfile:
        ...
        @njit  # instructs numba to optimize this function
        def myfunc(r):
        ...
        while tm.perf_counter() - wall < 10:
            ...
            result = myfunc(rep)
            now = tm.perf_counter()
            # write measured time to csv file: [wall clock in ms], [duration of each invocation run in ms]  
            ...
            print("Avg time, numpy_opt(numba): %f ms. result: %i" % (now - start, result[0,0,0]))
