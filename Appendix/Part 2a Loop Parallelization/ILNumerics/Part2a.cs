...
void workload_ILN_Seq_VP() {
    for (int i = 0; i < A.S[1]; i++) {
        ES[i] = sum(abs(sin(A[full, i])), dim: 0);
    }
}
void workload_ILN_Vect_VP() {
...
if (!File.Exists("values.csv")) {
    ...
    for (var start = DateTime.Now; DateTime.Now < start + TimeSpan.FromSeconds(10);) {
        ...
        var spent_ILN_vect = run(workload_ILN_Vect);
        var spent_ParFor = run(workload_ParFor);
        var spent_ILN_Seq_VP = run(workload_ILN_Seq_VP, awaitable: () => ES);
        var spent_ILN_Vect_VP = run(workload_ILN_Vect_VP, awaitable: () => EV);

        Console.Write($"{PartName} ILNseq: {spent_ILN_seq} ILNvect: {spent_ILN_vect} ms,");
        ...
        Console.Write($"{PartName} VP Vect: {spent_ILN_Vect_VP} ms"); check((double)maxall(abs(EV - CS)));
...
void draw() {
    ...
    var scene = new Scene {
        new PlotCube(twoDMode: true) {
            ...
            new LinePlot(tosingle(times[full,"0,2"]).T, tag: "ILNumerics  vectorized", lineWidth: 3, markerStyle: MarkerStyle.TriangleUp, lineColor: Color.Black, lineStyle: DashStyle.Solid ),
            new LinePlot(tosingle(times[full,"0,4"]).T, tag: "ILNumerics  for  loop,  VP ", lineWidth: 3, markerStyle: MarkerStyle.Dot, lineColor: Color.Red, lineStyle: DashStyle.Dotted),
            new LinePlot(tosingle(times[full,"0,3"]).T, tag: "Parallel.For", lineWidth: 3, markerStyle: MarkerStyle.Cross, markerColor: Color.Blue, lineColor: Color.Blue, lineStyle: DashStyle.PointDash),
            new LinePlot(tosingle(times[full,"0,5"]).T, tag: "ILNumerics  vectorized,  VP", lineWidth: 3, markerStyle: MarkerStyle.Dot, lineColor: Color.Black, lineStyle: DashStyle.Solid),
            new Legend() {
